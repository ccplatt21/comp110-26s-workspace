...
def input_guess(secret_word_len: int) -> str: 
    ...
    guess: str = input(f"Enter a {secret_word_len} character word: ") #local variable guess as an input to the prompting
...
# a while loop to loop throguh each index and emojify
    while idx < len(user_guess):
        if user_guess[idx] == secret_word[idx]: #if the letters at that index match, green box
            emoji += GREEN_BOX #assignment operator to add the green box to the emoji variable empty string
        elif contains_char(secret_word, user_guess[idx]): #if the letters match somewhere else, yellow box
            emoji += YELLOW_BOX
        else: 
            emoji += WHITE_BOX
...
def main(secret: str) -> None: 
    ...
    while n <= 6 and win == False: #the loop continues up to 6 turns and if they haven't won
